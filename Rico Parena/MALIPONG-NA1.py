...
class Tindahan:
    ...
    def save_products(self):
        with open("product_list.txt", "w") as file: # Deletes all contents to prevent duplication
            file.write("")
        with open("product_list.txt", "a") as file: # Adds all products back to the text file
            for pcode in self.rjp_product.keys():
...
class Customer:
    ...
    def customer_record(self, c_id, c_name, c_address):
    ...
    def display_customer(self):
        print("\nMga Suki")
        for customer_id, details in self.c_customer.items():
            print(str(customer_id) + "-" + str(details[0]) + " " + str(details[1]))

    def save_customers(self):
        with open("customer_list.txt", "w") as file: # Deletes all contents to prevent duplication
            file.write("")
        with open("customer_list.txt", "a") as file: # Adds all customers back to the text file
            for customer_id in self.c_customer.keys():
                cname, caddress = self.c_customer[customer_id]
                file.write(f"{customer_id},{cname},{caddress}\n")
            ...
            with open("customer_list.txt", "r") as file:
                for line in file:
                    ...
                    if line:
                        values = line.split(",")
                        if len(values) == 3:
                            cid, cname, caddress = values
                            customer1.customer_record(cid, cname, caddress)
                        else:
                            print(f"Invalid line format: {line}")
        except FileNotFoundError:
            print("Product list file not found.")
...
def admin_menu():
    ...
    while rpt == "Y":
        ...
        if choice == '1':
            ...
            customer1.customer_record(c_id, c_name, c_address)
